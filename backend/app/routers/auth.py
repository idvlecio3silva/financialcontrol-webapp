...
from app.core.security import (
    ...
    create_refresh_token, decode_token, get_client_ip
)
...
async def register(payload: RegisterRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    await log_action(db, user_id=user.id, action=AuditAction.CREATE, entity_type="user",
                     entity_id=str(user.id), ip_address=get_client_ip(request))
    return user
...
async def login(payload: LoginRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not user:
        await log_action(db, user_id=None, action=AuditAction.LOGIN_FAILED, entity_type="auth",
                         new_values={"email": payload.email}, ip_address=ip)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Credenciais inválidas")

    # Check lockout
    if user.locked_until and user.locked_until > datetime.now(timezone.utc):
        await log_action(db, user_id=user.id, action=AuditAction.LOGIN_LOCKED, entity_type="auth",
                         ip_address=ip)
    ...
    if not verify_password(payload.password, user.hashed_password):
        ...
        await log_action(db, user_id=user.id, action=AuditAction.LOGIN_FAILED, entity_type="auth",
                         new_values={"attempts": user.failed_login_attempts}, ip_address=ip)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Credenciais inválidas")
    ...
    await log_action(db, user_id=user.id, action=AuditAction.LOGIN, entity_type="auth", ip_address=ip)
...
async def refresh(payload: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    import uuid
    r = await db.execute(select(User).where(User.id == uuid.UUID(decoded["sub"])))
    user = r.scalar_one_or_none()
...
async def logout(request: Request, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    await log_action(db, user_id=current_user.id, action=AuditAction.LOGOUT, entity_type="auth",
